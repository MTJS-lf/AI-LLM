...
from src.layers.loss import CoSentLoss, ClsContrastLoss, RetriContrastLoss
...
class STEmbedder(EmbedderForTrain):
    ...
    def __init__(
        ...
        self.retri_contrst_loss = build_loss('retri_contrast', temperature=0.01, use_all_pair=True)
        ...
        if extend_pe:
            ...
            extended_pe = torch.nn.Embedding(max_length + sp, org_pe.embedding_dim, padding_idx=pad_idx)
            for start_idx in range(0, max_length + sp, org_pe.num_embeddings): # 迭代式地去复制,从而扩增embedding
            ...
            self.embedder.encoder.embeddings.position_ids = torch.arange(max_length + sp).expand((1, -1))
    ...
    def compute_retri_contrast_loss(self, text_ids: torch.Tensor, text_pos_ids: torch.Tensor, text_neg_ids: torch.Tensor = None,
                                    type: str = 'retri_contrast', **kwargs) -> dict[str, torch.Tensor]:
        text_embeddings = self.get_embedding(text_ids)
        text_pos_embeddings = self.get_embedding(text_pos_ids)
        text_neg_embeddings = self.get_embedding(text_neg_ids)
        ...
        if self.use_mrl:
            ...
            for num_feat in self.mrl_nesting_list:
                if text_neg_embeddings is not None:
                    ...
                    neg_emb = None
                loss += self.retri_contrst_loss(emb, pos_emb, neg_emb, **kwargs) / len(self.mrl_nesting_list)
        else:
            loss = self.retri_contrst_loss(text_embeddings, text_pos_embeddings, text_neg_embeddings, **kwargs)
